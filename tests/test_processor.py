...
def test_low_severity_event_is_recorded_without_incident(session):
    ...
    _event, incident, action, skipped, _replay = EventProcessor(session).process(payload)
...
def test_cooldown_suppresses_repeated_action(session):
    ...
    _event, incident, action, skipped, _replay = EventProcessor(session).process(
...
def test_health_sweep_queues_follow_up_for_stale_open_incident(session):
    ...
    _event, incident, _action, _skipped, _replay = EventProcessor(session).process(payload)
    assert incident is not None
